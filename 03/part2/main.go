...
....................571.......................720..........269...........885.............................902...........80...738..........975`
	...
	matrix := turnIntoMatrix(input)

	// the key number is the * position
	// "20": ["456", "234553"]
	starSymbols := map[string][]string {}
	...
	for i := 0; i < len(matrix); i++ {
		for j := 0; j < len(matrix[i]); {
			if _, err := strconv.Atoi(matrix[i][j]); err == nil {
				...
				for !foundWholeNumber {
					if len(matrix[i]) <= currentIndex {
						foundWholeNumber = true
						continue
					}
					if _, err := strconv.Atoi(matrix[i][currentIndex]); err == nil {
						number += matrix[i][currentIndex]
						currentIndex++
					} else {
						foundWholeNumber = true
					}
				...
				j += len(number)
				row, col, err := verifyPartNumber(i, j, number, matrix)
				...
				if matrix[row][col] == "*" {
					...
					starSymbols[key] = append(starSymbols[key], number)
				}
				continue;
			}
			j++;
...
func verifyPartNumber(row int, col int, number string, matrix[][]string) (int, int, string) {
	// verify left side
	posLeft := col - len(number)
	if (posLeft - 1) >= 0 {
		if isSymbol(matrix[row][posLeft-1]) {
			return row, posLeft - 1, "";
		}
	}
	...
	if (row > 0) {
		var rangeStart int
		if ((col - len(number)) - 1) >= 0 {
			rangeStart = col - len(number) - 1;
		} else {
			rangeStart = col - len(number)
		}
		...
		top := matrix[row-1][rangeStart:rangeEnd]
		for i, c := range top {
			if isSymbol(c) {
				return row - 1, rangeStart + i, "";
			}
		}
	}
	...
	if (row < len(matrix[row]) - 1) {
		...
		bottom := matrix[row + 1][rangeStart: rangeEnd]
...
func turnIntoMatrix(input string) [][]string {
	var matrix [][]string
