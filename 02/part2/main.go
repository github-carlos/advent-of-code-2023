...
func parseGameFromString(item string) Game {
	var game Game
	splitedGame := strings.Split(item, ":")
	id := strings.Split(splitedGame[0], " ")[1]
...
func parseSetFromString(item string) Set {
	var set Set
	cubesString := strings.Split(item, ",")

	for _, data := range cubesString {
		var cube Cube
		sd := strings.Split(strings.TrimSpace(data), " ")
		cube.color = sd[1]
		cube.quantity, _ = strconv.Atoi(sd[0])
