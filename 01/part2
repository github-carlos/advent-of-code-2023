...
53ldplzx`
	...
	splitedLines := strings.Split(input, "\n")
	...
	for _, line := range splitedLines {
		...
		for i, c := range line {
			...
			matched := regex.FindStringSubmatch(line[startWord:i + 1])

			if len(matched) > 0 {
				startWord = i;
				if firstNumber == 0 {
					firstNumber = numbersList[matched[0]]
				} else {
					secondNumber = numbersList[matched[0]]
				}
			}

			// is end of line
			if i == len(line) - 1 {
				fmt.Println("End of line", string(firstNumber), string(secondNumber))
				var value string
				if firstNumber != 0 && secondNumber != 0 {
					value = string(firstNumber) + string(secondNumber);
				} else {
					value = string(firstNumber) + string(firstNumber);
				}
				convValue, _ := strconv.Atoi(value)
				result += convValue;
			}
		}
	}
